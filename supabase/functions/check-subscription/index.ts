...
serve(async (req) => {
  ...
  try {
    ...
    if (subscriptions.data.length > 0) {
      ...
      let planName = 'Free';
      if (amount === 2900) planName = 'Pro';
      else if (amount === 9900) planName = 'Team';
    ...
    await supabaseClient.from("billing").upsert({
      ...
      plan_end_date: subscriptionData.subscription_end,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'user_id' });

    logStep("Updated billing data", subscriptionData);
